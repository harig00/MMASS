function dsbcouplinglog(as,offs)
% DSBCOUPLINGLOG(as,offs)
%
...
%
% Last modified by fjsimons-at-alum.mit.edu, 1/2/2008
...
for ondex=1:length(TH1)
  ...
  ylim(minmax(K-minb)+widr+[max(K-minb)-30 0])

  xlim([-skel(aN) skel(aN)])
  tixx{ondex}=0:tb(ondex):round(skel(aN)/10)*10;
  xl(ondex)=xlabel(xtag);
  yl(ondex)=ylabel(sprintf(['K_{ll''} (dB)']));
...
for ondex=1:length(TH2)
  ...
  % Somehow normalize this to the effective bandwidth as percentage
  % leaked to, from and within the effective bandwidth
  % No, do the log scaling but watch for the zeros
  ...
  xlim([-skel(aN) skel(aN)])
  tixx{ondex+length(TH1)}=0:tb2(ondex):round(skel(aN)/10)*10;
  xl(ondex+length(TH1))=xlabel(xtag);
  yl(ondex+length(TH1))=ylabel(sprintf(['K_{ll''} (dB)']));
  % Put in cap size labels
...
for ondex=1:length(ha)
  set(ha(ondex),'xtick',unique([-tixx{ondex} tixx{ondex}]))
end
set(th2,'FontS',8)
...
% But make sure the logo's show!
for indx=1:length(lah)
  top(lah(indx))
end
