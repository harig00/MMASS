...
% QUAKES=readCMT(fname,dirn,tbeg,tend,mblo,mbhi,depmin,depmax)
%
% Reads in CMT catalog of events (default format is that of jan76_dec10.ndk).
%
% INPUT:
%
...
% Last modified by fjsimons-at-alum.mit.edu, 04/14/2014

% Check to see if it's a demo case
if isempty(strfind(fname,'demo'))
  % Assign default values
  defval('fname','jan76_feb10.ndk')
...
elseif strcmp(fname,'demo3')
  % Get all earthquakes from CMT catalog for 1977-2010
  defval('dirn',fullfile(getenv('IFILES'),'CMT'))
  fname=fullfile(dirn,'quakes77_2010.mat');
  if exist(fname,'file')==2
    load(fname)
  else
    QUAKES=readCMT('jan76_feb10.ndk',[],datenum('1977/01/01 00:00:01'),...
		   [],[],[],[],[]);
    save(fname,'QUAKES')
  end
elseif strcmp(fname,'demo4')
  ...
  if exist(fname,'file')==2
    ...
    pre=readCMT('demo2');
    post=readCMT('jan76_feb10.ndk',[],datenum('1985/03/31 00:00:01'),...
		 [],[],[],[],[]);
